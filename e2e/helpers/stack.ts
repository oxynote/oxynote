...
// E2E_COMPOSE_EXTRA working — CI sets it to layer the buildx cache config
// in docker-compose.ci.yaml over the base file.
//
...
export function buildStack(): Promise<void> {
	return run("make", ["e2e-stack-build"], ROOT_DIR, "inherit")
}

export function stopStack(): Promise<void> {
	return run("make", ["e2e-stack-stop"], ROOT_DIR, "inherit")
}
...
export function startStack(): Promise<void> {
	return run("docker", ["compose", "up", "-d", "--wait"], E2E_DIR, "capture")
}
