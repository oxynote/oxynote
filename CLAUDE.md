...
```sh
make deps      # install web/ dependencies (pnpm install)
make setup     # web/ deps + prepare (lezer-promql build + nuxt prepare)
...
- **Front door**: Caddy on `:13321`. `/go/*` is path-stripped and proxied to core (`:8080`); everything else goes to auth-realtime (`:13321`). The frontend reaches the Go API via `NUXT_PUBLIC_GO_API_BASE_HTTP_URL` (`…:13321/go`) and auth/realtime via `NUXT_PUBLIC_NODEJS_API_BASE_*_URL`.
