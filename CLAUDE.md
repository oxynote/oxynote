...
- **Trust boundary**: core's `/api/...` routes require a session; core's `/api/x/...` and auth-realtime's `/api/internal/...` routes have no auth and must never be exposed by the reverse proxy (the Caddyfile blocks both at the front door) — they exist for service-to-service calls (auth-realtime ↔ core).
