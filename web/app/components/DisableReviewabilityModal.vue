...
const open = defineModel<(() => Promise<void>) | null>({
	default: null,
})
...
async function confirmDisable() {
	...
	const disableReviewability = open.value

	loading.value = true
	await delay(300) // show loading spinner for at least a moment
	await disableReviewability()
	open.value = null
				...
				<div class="flex min-h-0 flex-col gap-3">
					<ShadcnUiDialogHeader>
						<ShadcnUiDialogTitle class="text-base">
							{{
								$t(
									"editor.navbar.document-options.review-workflow.disable-confirm-modal.title",
								)
							}}
						</ShadcnUiDialogTitle>
						<ShadcnUiButton
					...
					<div class="flex flex-col gap-2 self-stretch">
						<ShadcnUiDialogDescription class="text-2sm">
							{{
								$t(
									"editor.navbar.document-options.review-workflow.disable-confirm-modal.description",
								)
							}}
