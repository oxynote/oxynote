...
import ParameterListItemHeaderType from "./ParameterListItemHeaderType.vue"
import { makeNode, mountNodeView } from "../../test-helpers/node-view"
import { SPLIT_DOCUMENTATION_PARAMETER_LIST_ITEM_HEADER_TYPE_NAME } from "../node-names"
import { t } from "~/components/test-helpers"

function mountHeader(overrides: Record<string, unknown> = {}) {
	return mountNodeView(ParameterListItemHeaderType, {
		node: makeNode(
			{ uid: "header-1" },
			{ typeName: SPLIT_DOCUMENTATION_PARAMETER_LIST_ITEM_HEADER_TYPE_NAME },
		),
		...overrides,
	})
}
