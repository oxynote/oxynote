...
- **`internal/document/hook/manager`** does the same for hooks: a row whose branch or document went NULL gets `Hook.Delete` (external teardown) before the row is dropped.
- **Ordering is inverted between create and delete.** Creating writes the row before the object (upload, duplication); deleting removes the object before the row. Both leave at most a row without an object, which the sweep resolves — an object without a row is invisible to every cleanup path.
