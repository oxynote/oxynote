...
func stubDataSource(typ datasource.Type) *datasource.DataSource {
...
func stubDB(ds *datasource.DataSource, fetchErr error) *DBMock {
	return &DBMock{
		FetchDataSourceFunc: func(_ context.Context, _ xid.ID, _ string) (*datasource.DataSource, error) {
			if fetchErr != nil {
				return nil, fetchErr
			}
...
func Test_NewHandler(t *testing.T) {
	...
	assert.Equal(t, datasource.NewExecutor(), hdl.executor)
}
...
func Test_Handler_CreateDataSource(t *testing.T) {
	hasCreatedDataSourceResp := func() check {
		return func(t *testing.T, db *DBMock, exec *ExecutorMock, rec *httptest.ResponseRecorder, _ *bytes.Buffer) {
			...
			assert.Equal(t, datasource.TypePrometheus, ds.Type)
	...
	cc := map[string]struct {
		...
		"Error returned by executor.TestConnection": {
			Exec: &ExecutorMock{
				TestConnectionFunc: func(_ context.Context, _ datasource.DataSource) (processor.ConnectionStatus, error) {
		...
		"Error returned by db.InsertDataSource": {
			DB: &DBMock{
				InsertDataSourceFunc: func(_ context.Context, _ *datasource.DataSource) error {
					return assert.AnError
				},
			},
			Exec: &ExecutorMock{
...
func Test_Handler_FetchDataSource(t *testing.T) {
	cc := map[string]struct {
		...
		"Successful fetch": {
			...
			ID: _testID.String(),
			Checks: checks(
				hasJSONResp(http.StatusOK, stubDataSource(datasource.TypePrometheus)),
				wasDBFetchDataSourceCalled(1),
			),
		},
	}
...
func Test_Handler_FetchDataSources(t *testing.T) {
	...
	cc := map[string]struct {
		...
		"Error returned by db.FetchDataSources": {
			DB: &DBMock{
				FetchDataSourcesFunc: func(_ context.Context, _ string) ([]datasource.DataSource, error) {
		...
		"Successful fetch": {
			DB: &DBMock{
				FetchDataSourcesFunc: func(_ context.Context, _ string) ([]datasource.DataSource, error) {
					return []datasource.DataSource{*stubDataSource(datasource.TypePrometheus)}, nil
				},
			},
			Checks: checks(
				hasJSONResp(http.StatusOK, []datasource.DataSource{*stubDataSource(datasource.TypePrometheus)}),
				wasDBFetchDataSourcesCalled(1),
			),
		},
	}
