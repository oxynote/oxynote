...
// IsJSONError checks whether the provided error has a concrete JSON
// error inside itself.
func IsJSONError(err error) bool {
	var (
		err1 *json.InvalidUnmarshalError
		err2 *json.MarshalerError
		err3 *json.UnmarshalTypeError
		err4 *json.UnsupportedTypeError
		err5 *json.UnsupportedValueError
	)

	return errors.As(err, &err1) ||
		errors.As(err, &err2) ||
		errors.As(err, &err3) ||
		errors.As(err, &err4) ||
		errors.As(err, &err5)
