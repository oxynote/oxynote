...
const (
	// FilterPrefix is a prefix that is used to group HTTP query
	// filters together.
	FilterPrefix = "filter-"

	// SortPrefix is a prefix that is used to group HTTP query
	// sort keys together.
	SortPrefix = "sort-"
)
...
type Query struct {
	...
	// be partial).
	// Filter's key must start with FilterPrefix in the request's query
	// parameters to be put in this map.
	Filters map[string]string `schema:"-" json:"filters,omitempty"`

	// SortKeys specifies a slice of fields that should be used
	// for sorting.
	// Sorting key must start with SortPrefix in the request's query
	// parameters to be put in this slice.
...
func ParseQuery(r *http.Request) (Query, error) {
	...
	for k := range r.Form {
		switch {
		case strings.HasPrefix(k, FilterPrefix):
			nk := strings.TrimPrefix(k, FilterPrefix)
			if nk == "" {
				continue
			}
			...
			qr.Filters[nk] = v
		case strings.HasPrefix(k, SortPrefix):
